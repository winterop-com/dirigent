...
dot names the system or the surface (`http`, `storage`, `docker`, `weather`), and the segment
...
class WeatherPlugin:
    ...
    def contribute(self) -> Contribution:
        return Contribution(
            ...
            operators=[WeatherObservationsOperator(), WeatherStationsOperator()],
        )
...
| `dirigent-<system>` | An adapter pack: one connection kind plus the blocks for one external system | `dirigent-weather` |
...
to level (`{"*": "debug"}`, `{"weather.*": "debug"}`), the most specific matching pattern wins,
