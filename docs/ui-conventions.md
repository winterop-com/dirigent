...
from the control's start edge and swaps to the end edge where starting there would put it off
screen, so a menu never scrolls sideways and never hangs off one. Only where the longest row is
wider than that cap does a row give way, and then it is the label that ellipses.
