...
ordinary storage blocks and `save_to` fields address it the way they address anything else,
and every document here declares `requires: {storage: [s3]}` so an instance without a
backend refuses it before storing it.

All four name one connection, `artifacts`, and it is the code the
...
| [s3-csv-report.yaml](s3-csv-report.yaml) | A report delivered to a bucket: the csv-report chain with `save_to` pointing one word away from scratch. |
