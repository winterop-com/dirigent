...
from dirigent_block_base.clock import (
    MAX_WAIT,
    MIN_SLEEP_POLL,
    MIN_WAIT,
    TimeSleepConfig,
...
def freeze(monkeypatch: pytest.MonkeyPatch, clock: str) -> None:
    ...
    monkeypatch.setattr("dirigent_block_base.clock.now_in", reading)
...
def test_the_clock_is_read_in_the_configured_zone() -> None:
    from dirigent_block_base.clock import now_in
...
async def test_the_last_sliver_of_a_wait_is_one_poke_and_not_a_flurry(
    ...
    monkeypatch.setattr("dirigent_block_base.clock.datetime", Held)
