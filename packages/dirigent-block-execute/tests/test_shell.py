...
from dirigent_block_execute.shell import ShellRunConfig, ShellRunOperator, ShellRunOutput
...
async def test_a_relative_work_root_becomes_an_absolute_workspace(
    ...
    Both the run's root and the attempt's workspace, because a block takes its working
    directory from one and the paths it hands a tool from the other.
    """
    ...
    root = local_ctx.as_context().work
    output = await run(local_ctx, ShellRunConfig(command="pwd"))
