"""Tests for the parquet family's plugin wiring."""

from pluginkit import PluginManager

from dirigent_block_parquet import ParquetBlocks, plugin
...
def test_the_family_contributes_the_codec_and_nothing_else() -> None:
    contribution = ParquetBlocks().contribute()
...
def test_the_family_registers_through_a_plugin_manager() -> None:
    ...
    manager.register(plugin, name="block-parquet")
...
def test_the_family_is_discovered_under_its_entry_point_name() -> None:
    ...
    assert registered >= 1
    assert manager.get_plugin("block-parquet") is not None
