"""Tests for the queue family's plugin wiring."""

from pluginkit import PluginManager

from dirigent_block_queues import QueuesBlocks, plugin
...
def test_the_family_contributes_its_blocks_and_nothing_else() -> None:
    contribution = QueuesBlocks().contribute()
    assert contribution.api_version == API_VERSION
    assert sorted(contribution.block_ids()) == QUEUES_BLOCKS
    assert sorted(kind.id for kind in contribution.connection_kinds) == ["kafka", "rabbitmq"]
...
def test_the_family_is_discovered_under_its_entry_point_name() -> None:
    ...
    assert registered >= 1
    assert manager.get_plugin("block-queues") is not None
