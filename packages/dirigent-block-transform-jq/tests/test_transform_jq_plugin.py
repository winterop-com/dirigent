...
from dirigent_block_transform_jq import TransformJqBlocks, plugin
...
def test_the_family_contributes_its_blocks_and_nothing_else() -> None:
    contribution = TransformJqBlocks().contribute()
    assert contribution.api_version == API_VERSION
    assert sorted(contribution.block_ids()) == TRANSFORM_JQ_BLOCKS
...
def test_every_block_declares_the_group_it_shelves_under() -> None:
    ...
    assert groups == TRANSFORM_JQ_GROUPS
...
def test_the_family_is_discovered_under_its_entry_point_name() -> None:
    ...
    assert registered >= 1
    assert manager.get_plugin("block-transform-jq") is not None
