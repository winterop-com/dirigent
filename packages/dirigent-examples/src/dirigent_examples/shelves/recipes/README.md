...
generated by `playground.generate`, a node that reaches nothing at all. Nothing needs the
unsafe-block allowlist: no recipe on this shelf runs code on a worker.

The recipes under **HTTP and webhooks** are the exception, and it is the honest one: a recipe
that is *about* an HTTP call makes one. They call the playground routes a dirigent instance
serves under `/api/v1/playground`, unauthenticated, so start an instance first and they reach
that and nothing on the public internet:
...
Every file in this section calls the instance's own playground, so `dg dev` has to be running.
...
| [http-post-json-body.yaml](http-post-json-body.yaml) | How do I POST a body assembled upstream, as structure rather than as text? |
