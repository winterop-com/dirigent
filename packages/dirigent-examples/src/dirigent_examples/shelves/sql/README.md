...
needs the engine package (`dirigent-block-duckdb`) and the parquet pack, and nothing else; the last
...
| [duckdb-parquet-to-report.yaml](duckdb-parquet-to-report.yaml) | The engine that reads files: a parquet artifact queried by DuckDB through a bound `read_parquet(:source)`, and the answer copied out as a csv artifact. Needs the `dirigent-block-duckdb` engine package and the parquet pack. |
